...
### 1. Access Success Test

```typescript
test("access gate - correct secret grants access", async ({ page }) => {
  const SECRET = "mysecret123"; // Must match VITE_ACCESS_SECRET in env
...
```typescript
test("access gate - wrong secret shows error", async ({ page }) => {
  await page.goto("/access");
  ...
  await expect(page.getByTestId("access-error")).toContainText("Wrong secret");
...
```typescript
test("access gate - logout clears access", async ({ page }) => {
  // First, grant access (via manual setup or separate test)
  ...
  // Logout
  await page.getByTestId("secret-logout-button").click();
