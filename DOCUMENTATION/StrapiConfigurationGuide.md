...
2. Under **Collection Types**, click **User**
3. You'll see existing fields including **role** (Relation to Strapi's Role collection)

**⚠️ Important**: You already have a `role` field set up as a **Relation** to Strapi's Role collection. **Keep this field as-is** - don't create a new Enumeration field.

#### Add Custom Fields (Application-Level Profile Fields)

Instead, add these additional fields for user profiles:
...
**Field 2: newsletterSubscribed (Boolean)**
...
**⚠️ Note**: Create this collection BEFORE creating roles, so you can configure Favorite permissions.
...
3. Configure the relation dialog:
   - **Left side**:
     - **Collection**: Favorite
     - **Field name**: Will auto-fill as `users_permission` (this is correct when connecting to User)
   - **Relation icon**: Click the **1st icon** (left, "has one")
   - **Right side**:
     - **Collection**: Select **User** from dropdown (⚠️ NOT Category)
     - **Field name**: Leave empty
   - This creates: "Favorite **has one** User"
4. Click **Finish**
...
3. Configure the relation dialog:
   ...
   - **Relation icon**: Click the **1st icon** (left, "has one")
   - **Right side**:
     - **Collection**: Select **Recipe** from dropdown
     - **Field name**: Leave empty or let auto-fill
   - This creates: "Favorite **has one** Recipe"
4. Click **Finish**
...
**Note**: `createdAt` and `updatedAt` are auto-generated by Strapi, in Content Manager (left sidebar)
Under Collection Types, click Favorite
When you view/create a favorite entry, scroll down to the bottom
You'll see createdAt and updatedAt fields automatically filled with timestamps
These are read-only (you can't edit them)
...
4. The "Add new Relation field" dialog opens:

   **Right side (User) - Do this first**:

   - Click the **User** dropdown (currently shows "Category")
   - Select **User** from the list
   - **Field name**: Leave empty
   - ⚠️ When you select User, the left field name will auto-fill as `users_permission`

   **Left side (Recipe)**:

   - **Field name**: **Change from `users_permission` to `author`** ✅
   - This is important for API clarity - `author` is more semantic than `users_permission`

   **Relation type**:

   - Click the **1st icon** from the left (leftmost, top row)
   - This represents: "Recipe **has one** User" and "User **has many** Recipes"
   - This creates a **One-to-Many** relationship

5. You should see text at bottom: **"Recipe has one User"**
6. Click **Finish**

7. click **Save** button (top left)
8. Wait for server restart (5-10 seconds)

✅ **Recipe now has author field!**

---

### Step 7.4: Create Chef Role in Strapi

**Purpose**: Create a Chef role that can manage recipes only
...
4. **Name**: `Chef`
5. **Description**: `Role for recipe creators with recipe management only`
6. Click **Save**

**Enable these permissions for Chef role**:
...
- ❌ `create`, `update`, `delete` - **Disabled** (only super user manages categories)
...
### Step 7.5: Create User Role in Strapi

**Purpose**: Create a User role for regular users (non-chefs)

1. In Settings → Roles, click **"Add new role"** button
2. **Name**: `User`
3. **Description**: `Role for regular users who can favorite recipes`
4. Click **Save**

**Enable these permissions for User role**:
...
- ✅ `find` - List recipes
- ✅ `findOne` - View single recipe
- ❌ `create`, `update`, `delete` - **Disabled** (only chefs and super user create recipes)
...
### Step 7.6: Superuser Access

**Purpose**: Understand superuser permissions in Strapi

**The superuser** (the admin account you created when setting up Strapi):

- ✅ Can create, read, update, and delete **everything**
- ✅ Can manage all collection types
- ✅ Can manage all roles and permissions
- ✅ Can modify any recipe, category, or user data
- ✅ Can assign and change user roles

**You use the superuser account for**:

- Managing categories
- Managing permissions
- Creating and assigning chef/user roles to other users
- Admin tasks and system configuration

✅ **Superuser already has all necessary permissions!**

---

### Step 7.7: Configure API Permissions for Public Users
...
### Step 7.8: Test Authentication API
...
### Step 7.9: Create Sample Users via Admin Panel
...
- **Password**: `Chef123!`
- **Role**: Select `Chef` role (the one you just created)
...
- **Password**: `Food123!`
- **Role**: Select `User` role (the one you just created)
...
### Step 7.10: Assign Author to Recipes (Optional)

To assign a chef as the author of a recipe:

1. In **Content Manager**, click **Recipe**
2. Edit an existing recipe or create a new one
3. Scroll down and find the **author** field
4. Click the field and select a chef user from the dropdown
5. Click **Save** and **Publish**

✅ **Recipe author assigned!**
